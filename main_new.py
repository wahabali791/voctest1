...
# Download the file
#gdown.download(f"{prefix+file_id}", 'downloaded_file.xlsx') # Download as Excel file

# Convert the Excel file to CSV
#excel_file_path = 'downloaded_file.xlsx'
#csv_file_path = 'completed_input.csv'
...
#df = pd.read_excel(excel_file_path)  # Read Excel file using pandas
df = pd.read_csv(csv_file_path)
df['Consent_arabic'] = df['Consent_arabic'].replace({'أوافق': 'I Accept', 'لا أوافق': 'I don\'t accept.'})

# Now, 'Consent_ar' column will have the English equivalents

#df.to_csv(csv_file_path, index=False)  # Convert and save as CSV

#print(f"Downloaded file converted to CSV: {csv_file_path}")
...
# Dial Code column
df['DialCode (1)'] = df.apply(apply_dialCode_logic, axis=1)
...
columns_to_drop = [
    ...
    'Phone_arabic','DialCode_arabic', 'Consent_arabic', 'Last Name_arabic','First Name_arabic','email_arabic','Gender_arabic',
    'Please share your feedback here...._arabic' , 'How frequently do you visit us?_arabic',
    'Do you have additional Feedback for us?_arabic','NPS_ar' , 'DialCode (2)' 
...
#df.rename(columns={'DialCode.1': 'DialCode (1)'}, inplace=True)
...
transformed_csv_file_path = 'combined.csv'
partial_combined_file_path= 'combined_partial.csv'
df1 = pd.read_csv(partial_combined_file_path)
#df.to_csv("combined_Completed.csv")
combined_df = pd.concat([df, df1], ignore_index=True)
combined_df = combined_df[combined_df['BUCode'].notna() & (combined_df['BUCode'] != '')]
combined_df.to_csv(transformed_csv_file_path, index=False)
